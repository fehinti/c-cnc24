...
block_t *block_new(char const *line, block_t *prev, machine_t const *machine)
{
  ...
  if (prev)
  { // this is not the first block, copy memory from previous one
...
data_t block_lambda(block_t *b, data_t t, data_t *s)
{
  ...
  // data_t ds_2 = b->prof->ds_2;
  data_t v_i = b->prof->v_i;
  ...
  { // deceleration
    ...
    if (dt_m > 0) {
      r = (f * dt_1 / 2.0) + (d / 2.0) * (pow(t, 2) + pow(t_2, 2)) + f * (dt_m + t - t_2) - d * t * t_2;
    } else {
      r = (d / 2.0) * (pow(t, 2) + pow(t_2, 2)) + v_i * (t - t_2) - d * t * t_2;
    }

    *s = f > 0 ? f + d * (t - t_2) :  fabs(d) * (t - t_2);
  }
  else
  {
    r = b->prof->l;
...
void block_acceleration(block_t *b)
{
  ...
  { // SHORT SEGMENT
    ...
    // if (b->next)
    //   b->next->prof->v_i = sqrt((2 * A * ds_1) + pow(v_i, 2));
  ...
  // point_set_xyz(vels, b->prof->v_i, b->prof->v_m, b->prof->v_f);
  // char *vel = NULL;
  // point_inspect(vels, &vel);
  // fprintf(stderr, "%s\n", vel);
...
void block_deceleration(block_t *b)
{
  ...
  if (((fabs(ds_2) > s_f) && (v_f < v_m)) || ((fabs(ds_1) > s_f) && (v_m < v_i)))
  { // SHORT SEGMENT \\ [D]
    ds_1 = 0;
    ds_2 = -s_f;
    ds_m = 0;
    // last segment/block in program has zero final velocity
    if (!(b->next) || !(block_type(b->next))) {
      v_f = 0;
    }
  ...
  { // LONG SEGMENT -> [v_f < v_m] // POSSIBLE CASE ends in [D]
    // v_m  =  sqrt(pow(v_f, 2) - (2 * A * ds_2));

    if (v_i > v_m)
    { // A -> v_i > v_m, s_0 < s_1 < s_2 < s_f               [DMD / MD]
      ds_1 = (pow(v_m, 2) - pow(v_i, 2)) / (2 * A);
      ds_m = s_f - (-ds_1 - ds_2);
      if (ds_m < 0)
      {
    ...
    { // B -> v_i < v_m, s_0 < s_1 < s_f < s_2   [AMD / AD]
      ds_1 = b->prof->ds_1;
      ds_m = s_f - (ds_1 - ds_2);
      if (ds_m < 0)
      {
  ...
  { // LONG SEGMENT -> [v_f > v_m] // POSSIBLE CASE ends in [A]
    if (v_i > v_m)
    { // A -> v_i > v_m, s_1 < s_0 < s_2 < s_f                  [DMA / DA]
      ds_1 = (pow(v_m, 2) - pow(v_i, 2)) / (2 * A);
      // ds_2 = b->prof->ds_2;
      ds_m = s_f - (-ds_1 + ds_2);
      if (ds_m < 0)
...
void block_abs_to_time(block_t *b)
{
  ...
  /* CONVERT FROM CURVLINIEAR ABSCISSA TO TIME ********************************************************/
  ...
  /* RESHAPE FOR DISRETE TIME *************************************************************************/
  dt = quantize(dt_1 + dt_m + dt_2, machine_tq(b->machine), &dq);
  if (dt_m > 0)
  { // Trapezoidal profile
    dt_m = dt_m + dq;
    v_m = v_m / (dt / (dt_1 + dt_2 + dt_m));
  }
  else if (dt_2 > 0)
  { // Triangular profile
    dt_2 = dt_2 + dq;
    v_f = v_f / (dt / (dt_1 + dt_2 + dt_m));
  }
  else
  {
    dt_1 = dt_1 + dq;
  ...
  // printf("\ndt1 of this block is => %.04f | Length => %.03f/%.03f | v_i => %.02f\n", dt_1, s_f, fabs(ds_1)+fabs(ds_2)+fabs(ds_m), v_i);
  // printf("dtm of this block is => %.04f | Length => %.03f/%.03f | v_m => %.02f\n", dt_m, s_f, fabs(ds_1)+fabs(ds_2)+fabs(ds_m), v_m);
  // printf("dt2 of this block is => %.04f | Length => %.03f/%.03f | v_f => %.02f\n", dt_2, s_f, fabs(ds_1)+fabs(ds_2)+fabs(ds_m), v_f);
  // printf("Total time of this block is => %.04f \n", dt_1 + dt_m + dt_2);

  // dt_1 acceleration/deceleration section
  if (dt_1 > 0)
    a = (dt_m > 0) ? (v_m - v_i) / dt_1 : (v_f - v_i) / dt_1;  // LOOK INTO THESE ACC. LOGIC
  else 
    a = 0;

  // dt_2 acceleration/deceleration section
  if (dt_2 > 0)
    d = (dt_m > 0) ? (v_f - v_m) / dt_2 : (v_f - v_i) / dt_2;  // LOOK INTO THESE ACC. LOGIC
  else 
    d = 0;
  // d = ((dt_2 > 0) && (dt_m > 0)) ? (v_f - v_m) / dt_2 : (v_f - v_i) / dt_2;
